...
| - [x] **T-28**  | **e2e_tunnel.rs** full flow                  | `crates/edgehub/tests/e2e_tunnel.rs` | *Desc* Start dnsd and edgehub, spawn `python -m http.server 8000`; run `ssh -i dev_key -R 8080:localhost:8000 certuser@127.0.0.1 -p2222`. Curl `http://demo.<slot>.fdns.run:8080` returns HTML index.<br>*AC* Test passes locally and in CI. |
