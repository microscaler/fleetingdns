...
impl SshKeyManager {
    pub fn new() -> Result<Self, SshKeyError> {
        ...
        Ok(Self {
            ...
            api_base_url: "https://api.edf.run".to_string(),
            auth_token: None,
        })
    }
    ...
    pub async fn request_key_pair(&self, session_ttl: u32) -> Result<SshKeyPair, SshKeyError> {
        info!("Requesting SSH key pair from remote API");
        
        let request = KeyRequest {
            key_type: "ed25519".to_string(),
            session_ttl,
            purpose: "tunnel-session".to_string(),
        };
        
        let mut req = self.api_client
            .post(&format!("{}/v1/ssh-keys", self.api_base_url))
            .json(&request);
        ...
        let key_pair = SshKeyPair {
            ...
            key_type: key_response.key_type,
        ...
        info!("SSH key pair received from API with fingerprint: {}", key_pair.fingerprint);
    ...
    pub async fn get_or_request_key_pair(&self, session_ttl: u32) -> Result<SshKeyPair, SshKeyError> {
    ...
    pub fn validate_key_pair(&self, key_pair: &SshKeyPair) -> Result<bool, SshKeyError> {
        ...
        if !key_pair.private_key.starts_with("-----BEGIN OPENSSH PRIVATE KEY-----") {
            return Ok(false);
        }
    ...
    pub async fn test_key_storage(&self) -> Result<SshKeyPair, SshKeyError> {
        ...
        info!("Test SSH key pair created with fingerprint: {}", key_pair.fingerprint);
        info!("Private key stored at: {}", private_key_path.display());
