...
Ephemeral DNS Forwarder (EDF) turns any developer workstation or CI job into a publicly addressable host **in one command** while remaining **temporary, secure, and zero‑config**.  It eliminates the friction of testing webhooks, OAuth callbacks, ACME flows, or multi‑tenant host‑based routing by creating a *30‑minute* DNS entry and a reverse tunnel that streams traffic back to the local service.  After TTL expiry or tunnel drop, all artefacts self‑destruct, leaving zero footprint.
