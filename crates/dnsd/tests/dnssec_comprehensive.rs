use dnsd::sign::{DnssecConfig, ProductionDnssecSigner, DnssecAlgorithm, AlertType};
...
fn test_dnssec_signature_caching() {
    ...
    let first_signature = signer.rrsig_record(&name, RecordType::A, 300, rrset).unwrap();
    let second_signature = signer.rrsig_record(&name, RecordType::A, 300, rrset).unwrap();
