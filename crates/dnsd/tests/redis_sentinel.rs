...
use dnsd::redis_sentinel::{PoolConfig, RedisSentinelClient, SentinelConfig, SentinelError};
...
fn test_sentinel_error_types() {
    ...
    assert_eq!(
        error.to_string(),
        "Invalid sentinel response: test response"
    );
...
async fn test_sentinel_config_serialization() {
    ...
    let deserialized: SentinelConfig =
...
async fn test_sentinel_stats_structure() {
    ...
    let deserialized: SentinelStats =
...
async fn test_sentinel_client_integration() {
    ...
    match RedisSentinelClient::new(config).await {
        Ok(client) => {
            ...
            let _master_addr = client
                .get_master_address()
                .await
                .expect("Failed to get master address");
