...
async fn test_cluster_config_creation() {
    ...
    assert!(
        config
            .nodes
            .contains(&"redis://redis-us-master-1:6379".to_string())
    );
    assert!(
        config
            .nodes
            .contains(&"redis://redis-apac-master-1:6379".to_string())
    );
    ...
    assert_eq!(
        config.discovery_config.refresh_interval,
...
async fn test_cluster_error_types() {
    ...
    assert_eq!(
        slot_mapping_error.to_string(),
        "Slot 12345 not mapped to any node"
    );
...
async fn test_bulk_operations_structure() {
    ...
    let operations = vec![
        ...
        (
            "slot2".to_string(),
            "127.0.0.2".parse::<Ipv4Addr>().unwrap(),
            1800,
        ),
        (
            "slot3".to_string(),
            "127.0.0.3".parse::<Ipv4Addr>().unwrap(),
            7200,
        ),
    ];
