...
impl RedisTestFixture {
    ...
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self { container: redis_container, pool })
    }
...
pub async fn with_shared_redis_container<F, Fut, T>(test_fn: F) -> Result<T, Box<dyn std::error::Error>>
where
    F: FnOnce(RedisPool) -> Fut,
    Fut: std::future::Future<Output = T>,
{
...
mod tests {
    ...
    async fn test_with_redis_container() {
        let result = with_redis_container(|pool| async move {
            ...
            let mut conn = conn.unwrap();
            let result: Result<String, _> = redis::cmd("SET").arg("test_key").arg("test_value").query_async(&mut *conn).await;
            assert!(result.is_ok());
            
            let result: Result<String, _> = redis::cmd("GET").arg("test_key").query_async(&mut *conn).await;
    ...
    async fn test_redis_operations() {
        let result = with_redis_container(|pool| async move {
            ...
            let _: () = redis::cmd("SET").arg("key1").arg("value1").query_async(&mut *conn).await.unwrap();
            let _: () = redis::cmd("SET").arg("key2").arg("value2").query_async(&mut *conn).await.unwrap();
            
            let value1: String = redis::cmd("GET").arg("key1").query_async(&mut *conn).await.unwrap();
            let value2: String = redis::cmd("GET").arg("key2").query_async(&mut *conn).await.unwrap();
