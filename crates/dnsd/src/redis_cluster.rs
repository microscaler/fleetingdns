...
impl RedisClusterClient {
    ...
    pub async fn new(config: ClusterConfig) -> Result<Self, ClusterError> {
        ...
        info!("Redis cluster client initialized with {} nodes", client.config.nodes.len());
        Ok(client)
    }
    ...
    pub async fn get_slot(&self, slot: &str) -> Result<Ipv4Addr, ClusterError> {
        ...
        self.update_node_health(&node_url, true, start.elapsed()).await;
        
        match result {
            Some(ip_str) => {
                ip_str.parse().map_err(|_| ClusterError::InvalidIpAddress(ip_str))
            }
    ...
    pub async fn bulk_set_slots(&self, operations: Vec<(String, Ipv4Addr, u64)>) -> Result<(), ClusterError> {
        ...
        for (slot, ip, ttl) in operations {
            let key_slot = self.calculate_slot(&slot);
            let node_url = self.get_node_for_slot(key_slot).await?;
            node_operations.entry(node_url).or_default().push((slot, ip, ttl));
        }
    ...
    async fn get_node_for_slot(&self, slot: u16) -> Result<String, ClusterError> {
        ...
        for cluster_slot in cluster_slots.iter() {
            if slot >= cluster_slot.start && slot <= cluster_slot.end {
                let node_url = format!("redis://{}:{}", cluster_slot.master.host, cluster_slot.master.port);
                ...
                for replica in &cluster_slot.replicas {
                    ...
                    if self.is_node_healthy(&replica_url).await {
                        warn!("Using replica {} for slot {} (master unhealthy)", replica_url, slot);
    ...
    async fn create_pool(&self, node_url: &str) -> Result<Pool<RedisConnectionManager>, ClusterError> {
    ...
    async fn discover_from_node(&self, node_url: &str) -> Result<(), ClusterError> {
        ...
        *self.cluster_slots.write().await = slots;
        info!("Discovered {} cluster slots from {}", self.cluster_slots.read().await.len(), node_url);
        
        Ok(())
    }

    /// Parse cluster slot information from Redis response
    fn parse_cluster_slot(&self, slot_info: redis::Value) -> Option<ClusterSlot> {
    ...
    async fn update_node_health(&self, node_url: &str, success: bool, response_time: Duration) {
        ...
        let health = node_health.entry(node_url.to_string()).or_insert(NodeHealth {
            is_healthy: true,
        ...
        if success {
            ...
            if health.consecutive_failures >= 3 {
                ...
                warn!("Node {} marked as unhealthy after {} failures", node_url, health.consecutive_failures);
            }
        }
    }

    /// Execute pipelined SET operations
    async fn execute_pipelined_sets(&self, node_url: &str, operations: Vec<(String, Ipv4Addr, u64)>) -> Result<(), ClusterError> {
        let pool = self.get_pool(node_url).await?;
        let mut conn = pool.get().await.map_err(ClusterError::Pool)?;
        ...
        for batch in operations.chunks(batch_size) {
            ...
            for (slot, ip, ttl) in batch {
                pipe.cmd("SET").arg(slot).arg(ip.to_string()).arg("EX").arg(*ttl);
            }
            
            let start = Instant::now();
            let _: Vec<()> = pipe.query_async(&mut *conn).await.map_err(ClusterError::Redis)?;
            
            self.update_node_health(node_url, true, start.elapsed()).await;
            debug!("Executed pipelined batch of {} operations on {}", batch.len(), node_url);
        }
                ...
                match timeout(Duration::from_secs(5), pool.get()).await {
                    Ok(Ok(mut conn)) => {
                        match redis::cmd("PING").query_async::<String>(&mut *conn).await {
                            Ok(_) => {
                                client.update_node_health(&node_url, true, start.elapsed()).await;
                            }
                            Err(_) => {
                                client.update_node_health(&node_url, false, start.elapsed()).await;
                            }
                        }
                    }
                    _ => {
...
mod crc16 {
    ...
    impl State<XMODEM> {
        ...
        pub fn update(&mut self, data: &[u8]) {
            for &byte in data {
                self.value = ((self.value << 8) ^ CRC16_XMODEM_TABLE[((self.value >> 8) ^ byte as u16) as usize]) & 0xFFFF;
            }
        }
