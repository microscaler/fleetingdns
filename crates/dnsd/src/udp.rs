...
mod tests {
    ...
    async fn test_handle_packet_with_signing() {
        ...
        unsafe { std::env::set_var("FDNS_HMAC_KEY", "test_signing_key"); }

        // Set up a test record
        let test_ip = Ipv4Addr::new(198, 51, 100, 1);
        if redis_cache::set_slot(&pool, "signed", test_ip, 300).await.is_err() {
            eprintln!("skipping test: redis set failed");
    ...
    async fn test_handle_packet_response_encoding() {
        ...
        if redis_cache::set_slot(&pool, "encode", test_ip, 300).await.is_err() {
            eprintln!("skipping test: redis set failed");
            redis_handle.abort();
            return;
        }
