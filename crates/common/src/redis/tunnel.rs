...
/// Lifecycle policy for tunnel teardown (FR-HUB-2).
///
/// Decided at tunnel creation by whoever stands the tunnel up — never by a
/// header from whoever connects (per-connection signals are unauthenticated,
/// spoofable, and conflict when a human tab and an agent share a tunnel).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TeardownPolicy {
    /// The tunnel dies on TTL expiry, explicit DELETE, or SSH-side
    /// disconnect only — never because nobody is currently viewing it.
    /// Deterministic for automation (e.g. agents driving Playwright open
    /// and close browsers between iterations). FleetingDNS default.
    #[default]
    TtlOnly,
    /// Additionally reap the hub slot listener once no viewer connection
    /// has been open for the idle threshold (human portal tabs; cylon
    /// PRD success criterion 6).
    ViewerIdle,
}
...
pub struct TunnelInfo {
    ...
    #[serde(default)]
    pub protected: bool,
    /// Teardown lifecycle policy (FR-HUB-2). Default `ttl_only` keeps
    /// older records deserializable and never GCs on viewer idleness.
    #[serde(default)]
...
/// Name of the cookie carrying the edge session grant token (FR-EDGE-3).
/// Shared contract between the control API (mints the cookie) and the
/// edge router (checks it before forwarding bytes on protected tunnels).
pub const SESSION_COOKIE_NAME: &str = "fdns_session";

/// Redis key for an edge session grant (FR-EDGE-3).
///
/// The control API mints an opaque token for the tunnel owner and writes
...
/// Check whether a session grant exists for `subdomain` + `token`.
///
/// Returns `Ok(true)` only while the grant key is live in Redis (the TTL
/// set at mint time enforces expiry — there is nothing to validate beyond
/// existence because the token is opaque and unguessable).
#[tracing::instrument(skip_all, fields(subdomain))]
pub async fn check_session_grant(
    pool: &RedisPool,
    subdomain: &str,
    token: &str,
    ...
    let exists: bool = conn
        .exists(session_grant_key(subdomain, token))
        .await
...
pub async fn store_tunnel_data(
    ...
    let ttl = if let Ok(expires) = tunnel_info
        .expires_at
...
pub async fn store_user_tunnel_lookup(
    ...
    tracing::info!(
        github_user_id,
        tunnel_id,
        "Updated user tunnel lookup in Redis"
    );
    Ok(())
}
            ...
            match serde_json::from_str::<UserTunnelLookup>(&data) {
                Ok(user_lookup) => {
                    ...
                    for tunnel_id in &user_lookup.tunnels {
                        ...
                        let tunnel_data: Option<String> = conn
                            .get(&tunnel_key)
                            .await
                            .map_err(|e| CacheError::RedisError(e.to_string()))?;
...
/// Get tunnel information by hub slot port from Redis.
///
/// Used by the hub's `tcpip_forward` handler to resolve the teardown policy
/// of the tunnel a slot belongs to. Same KEYS-based scan as
/// [`get_tunnel_by_subdomain`] (known tech debt — fine at dev-scale tunnel
/// counts, needs a secondary index before multi-tenant scale).
#[tracing::instrument(skip_all, fields(slot))]
pub async fn get_tunnel_by_slot(
    pool: &RedisPool,
    slot: u16,
) -> Result<Option<TunnelInfo>, CacheError> {
    let mut conn = pool
    ...
    for key in keys {
        let user_data: Option<String> = conn
            ...
            .await
            .map_err(|e| CacheError::RedisError(e.to_string()))?;
        let Some(data) = user_data else { continue };
        let Ok(user_lookup) = serde_json::from_str::<UserTunnelLookup>(&data) else {
            tracing::warn!(key = %key, "Failed to deserialize user tunnel lookup");
            continue;
        };
        for tunnel_id in &user_lookup.tunnels {
            let tunnel_data: Option<String> = conn
                .get(format!("tunnel:{tunnel_id}"))
                .await
                .map_err(|e| CacheError::RedisError(e.to_string()))?;
            let Some(data) = tunnel_data else { continue };
            match serde_json::from_str::<TunnelInfo>(&data) {
                Ok(tunnel_info) if tunnel_info.slot == slot => {
                    tracing::debug!(slot, tunnel_id = %tunnel_info.id, "Found tunnel for slot");
                    return Ok(Some(tunnel_info));
                }
                Ok(_) => {}
                Err(e) => {
    ...
    tracing::debug!(slot, "No tunnel found for slot");
    Ok(None)
}
