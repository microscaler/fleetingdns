...
impl UsageTracker {
    ...
    pub async fn get_quota_limits(&self, _user_id: &str) -> ApiResult<QuotaLimits> {
        // TODO: Get from database - simplified for now
    ...
    pub async fn check_quota(
        ...
        amount: i64,
    ) -> ApiResult<QuotaEnforcementResult> {
        ...
        let (current_usage, limit) = match quota_type {
            ...
            QuotaType::TunnelCreation => (
                usage.tunnels_created_count,
                limits.tunnel_creation_limit as i64,
            ),
            QuotaType::DnsProvisioning => (
                usage.dns_operations_count,
                limits.dns_provisioning_limit as i64,
            ),
            QuotaType::ConcurrentTunnels => (
                usage.active_tunnels_count,
                limits.max_concurrent_tunnels as i64,
            ),
            QuotaType::DataTransfer => (
                usage.data_transferred_mb,
                limits.data_transfer_limit_mb.unwrap_or(1024),
            ),
            QuotaType::CertificateIssuance => (
                usage.certificates_issued_count,
                limits.certificate_issuance_limit.unwrap_or(100) as i64,
            ),
        };
        ...
        Ok(QuotaEnforcementResult {
            ...
            quota_exceeded: if allowed {
                None
            } else {
                Some(quota_type.clone())
            },
            message: if allowed {
                format!(
                    "Operation allowed. Remaining quota: {}",
                    remaining.unwrap_or(0)
                )
            } else {
                format!(
                    "Quota exceeded for {quota_type:?}. Limit: {limit}, Current: {current_usage}, Requested: {amount}"
                )
            },
        })
    }
...
impl QuotaEnforcementMiddleware {
    ...
    pub async fn enforce_api_quota(&self, user_id: &str) -> ApiResult<QuotaEnforcementResult> {
        let result = self
            .usage_tracker
            .check_quota(user_id, QuotaType::ApiCalls, 1)
            .await?;

        if result.allowed {
            self.usage_tracker
                .record_usage(user_id, QuotaType::ApiCalls, 1)
                .await?;
        }
    ...
    pub async fn enforce_tunnel_creation_quota(
        &self,
        user_id: &str,
    ) -> ApiResult<QuotaEnforcementResult> {
        let result = self
            .usage_tracker
            .check_quota(user_id, QuotaType::TunnelCreation, 1)
            .await?;

        if result.allowed {
            self.usage_tracker
                .record_usage(user_id, QuotaType::TunnelCreation, 1)
                .await?;
        }
    ...
    pub async fn enforce_dns_quota(&self, user_id: &str) -> ApiResult<QuotaEnforcementResult> {
        let result = self
            .usage_tracker
            .check_quota(user_id, QuotaType::DnsProvisioning, 1)
            .await?;

        if result.allowed {
            self.usage_tracker
                .record_usage(user_id, QuotaType::DnsProvisioning, 1)
                .await?;
        }
    ...
    #[allow(dead_code)]
    pub async fn enforce_concurrent_tunnels_quota(
        &self,
        _user_id: &str,
        current_count: i64,
    ) -> ApiResult<QuotaEnforcementResult> {
        let result = self
            .usage_tracker
            .check_quota(_user_id, QuotaType::ConcurrentTunnels, current_count)
            .await?;
        Ok(result)
    }
    ...
    #[allow(dead_code)]
    pub async fn enforce_data_transfer_quota(
        &self,
        _user_id: &str,
        ...
        let result = self
            .usage_tracker
            .check_quota(_user_id, QuotaType::DataTransfer, mb_transferred)
            .await?;

        if result.allowed {
            self.usage_tracker
                .record_usage(_user_id, QuotaType::DataTransfer, mb_transferred)
                .await?;
        }
    ...
    #[allow(dead_code)]
    pub async fn enforce_certificate_quota(
        &self,
        _user_id: &str,
    ) -> ApiResult<QuotaEnforcementResult> {
        let result = self
            .usage_tracker
            .check_quota(_user_id, QuotaType::CertificateIssuance, 1)
            .await?;

        if result.allowed {
            self.usage_tracker
                .record_usage(_user_id, QuotaType::CertificateIssuance, 1)
                .await?;
        }
...
impl ServicePlanRateLimiter {
    ...
    pub async fn can_create_tunnel(&self, user_id: &str) -> ApiResult<bool> {
        let result = self
            .quota_enforcement
            .enforce_tunnel_creation_quota(user_id)
            .await?;
        Ok(result.allowed)
    }
...
impl QuotaInfo {
    ...
    pub fn get_usage_percentage(&self, quota_type: QuotaType) -> f64 {
        let (current, limit) = match quota_type {
            QuotaType::ApiCalls => (
                self.usage.api_calls_count,
                self.limits.api_rate_limit as i64,
            ),
            QuotaType::TunnelCreation => (
                self.usage.tunnels_created_count,
                self.limits.tunnel_creation_limit as i64,
            ),
            QuotaType::DnsProvisioning => (
                self.usage.dns_operations_count,
                self.limits.dns_provisioning_limit as i64,
            ),
            QuotaType::ConcurrentTunnels => (
                self.usage.active_tunnels_count,
                self.limits.max_concurrent_tunnels as i64,
            ),
            QuotaType::DataTransfer => (
                self.usage.data_transferred_mb,
                self.limits.data_transfer_limit_mb.unwrap_or(1024),
            ),
            QuotaType::CertificateIssuance => (
                self.usage.certificates_issued_count,
                self.limits.certificate_issuance_limit.unwrap_or(100) as i64,
            ),
        };
...
mod tests {
    ...
    async fn test_usage_reset_with_real_database() {
        ...
        // Record some usage
        tracker
            .record_usage(user_id, QuotaType::ApiCalls, 10)
            .await
            .unwrap();
    ...
    async fn test_quota_checking_allowed() {
        ...
        tracker
            .record_usage(user_id, QuotaType::ApiCalls, 50)
            .await
            .unwrap();
    ...
    async fn test_quota_checking_exceeded() {
        ...
        tracker
            .record_usage(user_id, QuotaType::ApiCalls, 15)
            .await
            .unwrap();
        ...
        tracker
            .record_usage(user_id, QuotaType::ApiCalls, 1000)
            .await
            .unwrap();
        let result = tracker.check_quota(user_id, QuotaType::ApiCalls, 1).await;
        assert!(result.is_ok());
    ...
    async fn test_quota_info_creation() {
        ...
        rate_limiter
            .usage_tracker
            .record_usage(user_id, QuotaType::ApiCalls, 25)
            .await
            .unwrap();
